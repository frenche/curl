                                              ...
                                              char **outptr, size_t *outlen)
{
  ...
  if(krb5->context == GSS_C_NO_CONTEXT) {
    ...
    if(GSS_ERROR(gss_major_status)) {
      Curl_gss_log_error(data, "gss_import_name() failed: ", gss_major_status,
                         gss_minor_status);
                                                  ...
                                                  size_t *outlen)
{
  ...
  if(GSS_ERROR(gss_major_status)) {
    Curl_gss_log_error(data, "gss_unwrap() failed: ", gss_major_status,
                       gss_minor_status);
  ...
  gss_major_status = gss_wrap(&gss_minor_status, krb5->context, 0,
                              ...
                              &output_token);
  if(GSS_ERROR(gss_major_status)) {
    Curl_gss_log_error(data, "gss_wrap() failed: ", gss_major_status,
                       gss_minor_status);
